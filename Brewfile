tap "buo/cask-upgrade"
tap "dart-lang/dart"
tap "dotenvx/brew"
tap "getstream/stream-cli", "https://github.com/GetStream/stream-cli"
tap "grishka/grishka"
tap "homebrew-ffmpeg/ffmpeg"
tap "homebrew/bundle"
tap "homebrew/cask"
tap "homebrew/core"
tap "homebrew/services"
tap "kyeshmz/personal"
tap "labstreaminglayer/tap"
tap "leoafarias/fvm"
tap "mongodb/brew"
tap "nrlquaker/createzap"
tap "oven-sh/bun"
tap "planetscale/tap"
tap "pulumi/tap"
tap "simonw/llm"
tap "sst/tap"
tap "stripe/stripe-cli"
tap "supabase/tap"
tap "wyattjoh/stable"
tap "xcodesorg/made"
tap "yakitrak/yakitrak"
brew "act"
brew "giflib"
brew "xz"
brew "arduino-cli"
brew "openssl@3"
brew "aria2"
# Build deps for mise's ruby/python compiles (config.toml sets python.compile).
# Listed explicitly so `brew autoremove` cannot take them: they arrived as
# dependencies of pyenv/ruby-install, and removing those autoremoved autoconf.
brew "autoconf"
brew "libyaml"
brew "readline"
brew "sqlite"
brew "pkg-config"
brew "awscli"
brew "bento4"
brew "cairo"
brew "openjdk"
brew "python@3.13"
brew "bfg"
brew "bitwarden-cli"
brew "ccusage"
brew "chezmoi"
brew "clang-format"
brew "cmake"
brew "cocoapods"
brew "coreutils"
brew "deno"
brew "dos2unix"
brew "terminal-notifier"
brew "fastlane"
brew "lame"
brew "ffmpeg"
brew "firebase-cli"
brew "flyctl"
brew "gawk"
brew "gcc"
brew "webp"
brew "gh"
brew "git"
brew "git-filter-repo"
brew "git-lfs"
brew "git-quick-stats"
brew "gitlab-ci-local"
brew "glab"
brew "gnu-sed"
brew "gnupg"
brew "go"
brew "googleworkspace-cli"
brew "pango"
brew "librsvg"
brew "graphviz"
brew "pkgconf"
brew "hdf5"
brew "hidapi"
brew "http-server"
brew "imagemagick"
brew "jj"
brew "jpeg"
brew "jq"
brew "libimobiledevice"
brew "libomp", link: true
brew "libraw"
brew "librealsense"
brew "libsoundio"
brew "livekit"
brew "livekit-cli"
brew "llvm"
brew "make"
brew "mas"
brew "mise"
brew "mkcert"
brew "mongosh"
brew "yt-dlp"
brew "mpv"
brew "ncdu"
brew "neovim"
brew "netcat"
brew "nss"
brew "openapi-generator"
brew "openblas"
brew "ripgrep"
brew "opencode"
brew "opentofu"
brew "pipx"
brew "pixi"
brew "pnpm"
brew "poppler"
brew "portaudio"
brew "postgresql@14", restart_service: :changed
brew "protobuf"
brew "protobuf@21"
brew "pscale"
brew "pulumi"
brew "python-setuptools"
brew "python@3.11"
brew "tcl-tk@8"
brew "python-tk@3.11"
brew "tcl-tk"
brew "python-tk@3.12", link: false
brew "python-tk@3.13"
brew "qt"
brew "qt@5"
brew "railway"
brew "redis"
brew "rename"
brew "rsgain"
brew "rustup"
brew "rye"
brew "scrcpy"
brew "snapcraft"
brew "sox"
brew "ssh-copy-id"
brew "sshpass"
brew "starship"
brew "swiftformat"
brew "tectonic"
brew "telnet"
brew "terraform"
brew "tmux"
brew "tree"
brew "uv"
brew "vercel-cli"
brew "virtualenv"
brew "wget"
brew "xcbeautify"
brew "xcode-build-server"
brew "yarn"
brew "youtube-dl"
brew "zlib"
brew "zplug"
brew "dart-lang/dart/dart"
brew "dotenvx/brew/dotenvx"
brew "getstream/stream-cli/stream-cli"
brew "labstreaminglayer/tap/lsl"
brew "labstreaminglayer/tap/labrecorder"
brew "leoafarias/fvm/fvm"
brew "mongodb/brew/mongodb-community"
brew "simonw/llm/ttok"
brew "stripe/stripe-cli/stripe"
brew "supabase/tap/supabase"
brew "wyattjoh/stable/claude-code-notification"
brew "xcodesorg/made/xcodes"
brew "yakitrak/yakitrak/notesmd-cli"
cask "adobe-creative-cloud"
cask "alt-tab"
cask "android-file-transfer"
cask "android-studio"
cask "angry-ip-scanner"
cask "anydesk"
cask "appcleaner"
cask "arduino-ide"
cask "audacity"
cask "authy"
cask "autodesk-fusion"
cask "balenaetcher"
cask "bambu-studio"
cask "betterdiscord-installer"
cask "bitwarden"
cask "blackhole-16ch"
cask "blackhole-2ch"
cask "blender"
cask "camo-studio"
cask "cap"
cask "chatgpt"
cask "cheatsheet"
cask "claude"
cask "cursor"
cask "cyberduck"
cask "cycling74-max"
cask "dbeaver-community"
cask "discord"
cask "displaylink"
cask "djv"
cask "docker-desktop"
cask "dropbox"
cask "expo-orbit"
cask "figma"
cask "firefox"
cask "focusrite-control"
cask "fork"
cask "gcloud-cli"
cask "gfxcardstatus"
cask "ghostty"
cask "google-chrome"
cask "google-drive"
cask "granola"
cask "handbrake-app"
cask "imageoptim"
cask "karabiner-elements"
cask "kicad"
cask "lastpass"
cask "lightburn"
cask "loom"
cask "losslesscut"
cask "mediainfo"
cask "microsoft-auto-update"
cask "microsoft-office"
cask "ndi-tools"
cask "grishka/grishka/neardrop"
cask "ngrok"
cask "notion"
cask "obs"
cask "obsidian"
cask "ogdesign-eagle"
cask "openvpn-connect"
cask "parsec"
cask "preform"
cask "processing"
cask "protonvpn"
cask "prusaslicer"
cask "qlmarkdown"
cask "qlstephen"
cask "quicklook-video"
cask "raycast"
cask "rectangle"
cask "rotato"
cask "saleae-logic"
cask "screen-studio"
cask "shutter-encoder"
cask "sketch"
cask "slack"
cask "spotify"
cask "steam"
cask "steamcmd"
cask "tailscale-app"
cask "telegram"
cask "the-unarchiver"
cask "ticktick"
cask "touchdesigner"
cask "vezer"
cask "vlc"
cask "vmware-fusion"
cask "whatsapp"
cask "whispering"
cask "wireshark-app"
cask "zerotier-one"
cask "zoom"
cask "zotero"
mas "Flow", id: 1423210932
mas "Keynote", id: 409183694
mas "LINE", id: 539883307
mas "MARKETSPEED", id: 946680495
mas "MIT Atlas", id: 1498222646
mas "Numbers", id: 409203825
mas "Perplexity", id: 6714467650
mas "Timery", id: 1425368544
mas "Toggl Track", id: 1291898086
mas "Transporter", id: 1450874784
mas "Webp Converter", id: 1527716894
vscode "4ops.terraform"
vscode "alexisvt.flutter-snippets"
vscode "anseki.vscode-color"
vscode "anysphere.cursorpyright"
vscode "anysphere.pyright"
vscode "astro-build.astro-vscode"
vscode "asvetliakov.vscode-neovim"
vscode "bierner.color-info"
vscode "biomejs.biome"
vscode "bradlc.vscode-tailwindcss"
vscode "clinyong.vscode-css-modules"
vscode "csharpier.csharpier-vscode"
vscode "csstools.postcss"
vscode "dart-code.dart-code"
vscode "dart-code.flutter"
vscode "davidwoo.flutter-auto-import"
vscode "dbaeumer.vscode-eslint"
vscode "donjayamanne.githistory"
vscode "dotjoshjohnson.xml"
vscode "dtoplak.vscode-glsllint"
vscode "esbenp.prettier-vscode"
vscode "felixangelov.bloc"
vscode "formulahendry.auto-rename-tag"
vscode "gaetschwartz.build-runner"
vscode "google.geminicodeassist"
vscode "hunghg255.css-2-tailwindcss"
vscode "kevinrose.vsc-python-indent"
vscode "luanpotter.dart-import"
vscode "lukepighetti.build-runner"
vscode "mathiasfrohlich.kotlin"
vscode "mhutchie.git-graph"
vscode "ms-python.debugpy"
vscode "ms-python.isort"
vscode "ms-python.python"
vscode "ms-python.vscode-pylance"
vscode "ms-vscode-remote.remote-containers"
vscode "ms-vscode-remote.remote-ssh"
vscode "ms-vscode-remote.remote-ssh-edit"
vscode "ms-vscode.cpptools"
vscode "ms-vscode.remote-explorer"
vscode "ms-vsliveshare.vsliveshare"
vscode "ms-vsliveshare.vsliveshare-pack"
vscode "naco-siren.gradle-language"
vscode "njpwerner.autodocstring"
vscode "octref.vetur"
vscode "pflannery.vscode-versionlens"
vscode "platformio.platformio-ide"
vscode "prisma.prisma"
vscode "rangav.vscode-thunder-client"
vscode "redhat.vscode-yaml"
vscode "robert-brunhage.flutter-riverpod-snippets"
vscode "searking.preview-vscode"
vscode "seatonjiang.gitmoji-vscode"
vscode "silvenon.mdx"
vscode "slevesque.shader"
vscode "steoates.autoimport"
vscode "stringham.copy-with-imports"
vscode "styled-components.vscode-styled-components"
vscode "syler.sass-indented"
vscode "unifiedjs.vscode-mdx"
vscode "vinicioslc.adb-interface-vscode"
vscode "vintharas.learn-vim"
vscode "vscjava.vscode-gradle"
vscode "waderyan.gitblame"
vscode "wokwi.wokwi-vscode"
vscode "xyc.vscode-mdx-preview"
vscode "yoavbls.pretty-ts-errors"
vscode "yzhang.markdown-all-in-one"
vscode "zarifprogrammer.tailwind-snippets"
vscode "zhuangtongfa.material-theme"
cargo "basalt-tui"
cargo "rustlings"
cargo "tauri-cli"
uv "qmk", with: ["pip"]
